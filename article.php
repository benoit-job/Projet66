...
if(isset($_POST['delete_article']))
    {
        ...
        $count = $bdd->prepare('SELECT count(id) AS cpt FROM article');
        $count->execute();
        $tcount = $count->fetchAll();

        $page = $_GET['page'];
        $nbr_element_par_page =2;
        $nbr_pages = ceil($tcount[0]['cpt']/ $nbr_element_par_page);
        $debut = ($page-1) * $nbr_element_par_page;
        

        
        $recupArticle = $bdd->prepare("SELECT * FROM article ORDER BY id DESC LIMIT $debut, $nbr_element_par_page");

        $recupArticle->execute();

        $article = $recupArticle->fetchAll();
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-md-6">
                            <div class="product__item">
                                <div class="product__item__pic set-bg" data-setbg="avatars/<?= $article[$i]['image'] ?>">
                                    <div class="label new">New</div>
                                    <ul class="product__hover">
                                    <li><a href="edit.php?id=<?= $article[$i]['id']; ?>"><span class="icon_pencil-edit"></span></a></li>
                                    <li>
                                            <form action="?id=<?= $article[$i]['id']; ?>" method="post">
                                        ...
                                        <li><a href="avatars/<?= $article[$i]['image'] ?>" class="image-popup"><span class="arrow_expand"></span></a></li>
                                        <li>
                                            <form action="" method="post" enctype="multipart/form-data">
                                                <input type="hidden" value="<?= $article[$i]['title']; ?>" name="title">
                                                <input type="hidden" value="<?= $article[$i]['prix']; ?>" name="prix">
                                                <input type="hidden" value="1" name="quantity">
                                                <input type="hidden" value="<?= $article[$i]['image'] ?>" name="image">
                                ...
                                <div class="product__item__text">
                                    <h6><a href="show.php?id=<?= $article[$i]['id']; ?>"><?php echo $article[$i]['title']; ?></a></h6>
                                    <div class="rating">
                                    ...
                                    <div class="product__price"><?php echo number_format($article[$i]['prix'],2); ?> FCFA</div>
                                </div>
                            </div>
                        </div>
                        <?php
